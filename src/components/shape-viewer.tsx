  ...
  (
    ...
    const insetFace = useCallback(
      (face: number[], gap: number) => {
        ...
        const avgEdge =
          edgeLens.reduce((acc, l) => acc + l, 0) / edgeLens.length
            ...
            setTimeout(() => {
              try {
                if (
                  ...
                  if (
                    values.length === 16 &&
                    window.x3dom?.fields?.SFMatrix4f
                  ) {
    ...
    useEffect(() => {
      ...
      const geometryGroup =
        containerRef.current.querySelector('#geometry-group')
      if (geometryGroup) {
      ...
      if (mode === 'motion' && speed > 0) {
        ...
        const animate = (currentTime: number) => {
          ...
          if (animParams.pauseDuration > 0 && pauseStartTimeRef.current > 0) {
            const pauseElapsed =
              (currentTime - pauseStartTimeRef.current) / 1000
